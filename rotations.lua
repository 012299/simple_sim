...
function settings:calc_haste_val_3tp(haste)
    ...
    local bob_cd = math.floor(settings.Settings.BOB_CD / brew_gen_sec)/rotation_duration*rotation_duration +3 --Energy starvation only occurs during the first 3 casts (ks/tp) of a cycle if that cycle has to be started without BoB up
